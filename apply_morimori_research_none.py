#!/usr/bin/env python3
"""
Researchページ（完全未適用5ページ）にもりもり版テンプレートを一括適用するスクリプト
...
def extract_hero_info(html):
    """Extract hero-shelf/ornament, hero-title, hero-lead/sub."""
    shelf = re.search(r'<div class="hero-(?:shelf|ornament)">(.*?)</div>', html, re.DOTALL)
    title = re.search(r'<h1 class="hero-title">(.*?)</h1>', html, re.DOTALL)
    lead = re.search(r'<p class="hero-(?:lead|sub)">(.*?)</p>', html, re.DOTALL)
    return {
        'shelf': shelf.group(1).strip() if shelf else 'RESEARCH',
        'shelf_class': 'hero-shelf' if (shelf and 'hero-shelf' in shelf.group(0)) else 'hero-ornament',
...
def get_related_slugs(current_slug):
    """Get 4 related page slugs (excluding self)."""
    related = [s for s in ALL_RELATED_SLUGS if s != current_slug]
    return related[:4]


def build_graph_nodes(hero_info, sections, current_slug):
    """Build Knowledge Graph nodes and edges for research pages."""
    nodes = []
    edges = []

    # Self node - article title (strip HTML tags)
    name = re.sub(r'<[^>]+>', '', hero_info['title']).strip()
    # Shorten for display
    ...
    for i, sec in enumerate(sections[:5]):
        ...
        label = sec['title'][:8]
        x, y = sec_positions[i]
        nodes.append(f"    {{id:'{sid}',label:'{label}',x:{x},y:{y},r:14,color:'#b8900a',cat:'section',url:'#sec{sec_num}',desc:'{sec['title']}'}}")
        edges.append(f"    {{from:'self',to:'{sid}'}}")

    # Related article nodes (4)
...
def build_qa_links(sections):
    """Build Quick Action links from first 5 sections."""
    icons = ["🔍", "⏰", "🧭", "🔄", "📖", "🔗"]
    links = []
    for i, sec in enumerate(sections[:5]):
        num = f"{i+1:02d}"
...
def process_page(filepath):
    """Apply morimori template to a single research page."""
    ...
    # Match padding:60px 24px 52px or padding:60px 24px etc.
    html = re.sub(
    ...
    old_hero = re.search(
        r'(<!-- [^>]*HERO[^>]* -->\s*)?<div class="hero">\s*'
        r'(?:<div class="hero-(?:shelf|ornament)">.*?</div>\s*)?'
        r'(?:<h1 class="hero-title">.*?</h1>\s*)?'
        r'(?:<p class="hero-(?:lead|sub)">.*?</p>\s*)?'
        r'</div>',
        html, re.DOTALL
    )
...
<div class="hero">
  <div class="hero-3col">
    ...
    <div class="hero-col-center">
      <div class="{hero['shelf_class']}">{hero['shelf']}</div>
  ...
  <div class="cockpit-panel">
    ...
    <div class="cockpit-tree">
      ...
      <div class="ct-children">
        <div class="ct-item ct-folder"><span class="ct-icon">&#128194;</span><a href="../">論考の棚</a><span class="ct-count">17</span></div>
        <div class="ct-children">
