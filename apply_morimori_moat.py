#!/usr/bin/env python3
"""
Moatセクションの全ページにもりもり版テンプレートを一括適用するスクリプト。
参考: apply_morimori_people.py

差分:
- 親フォルダ: 概念の棚 (5)
- ヒーロー中央: hero-shelf + hero-title + hero-lead（hero-name-enなし）
- Knowledge Graph の legend: この記事 / セクション / 関連概念
- related nodes: 同じmoatセクションの他4ページ（固定マップ）
"""
...
MOAT_DIR = os.path.dirname(os.path.abspath(__file__)) + "/moat"

# Moat5ページとその日本語名（関連ノード用）
MOAT_PAGES = {
    "brand-power": "ブランド力",
...
def extract_hero_info(html):
    """Extract hero-shelf, hero-title, hero-lead (moat版)."""
    ...
    return {
        'shelf': shelf.group(1).strip() if shelf else '概念の棚',
...
def build_graph_nodes(hero_info, sections, slug):
    """Build Knowledge Graph nodes/edges for moat page."""
    nodes = []
    edges = []
    # Self node - use the first line of title (before <br>) as label
    raw_title = re.sub(r'<br\s*/?>.*', '', hero_info['title'], flags=re.DOTALL)
    name = re.sub(r'<[^>]+>', '', raw_title).strip()
    if len(name) > 12:
        name = name[:12]
    nodes.append(f"    {{id:'self',label:'{name}',x:0,y:0,r:22,color:'#d4aa22',cat:'self',url:null,desc:'この記事'}}")

    # Section nodes - dynamic from actual sections
    positions = [(-110, -70), (110, -60), (130, 30), (-90, 60), (20, 110), (-140, 20)]
    for i, sec in enumerate(sections[:6]):
        num = f"{i+1:02d}"
        sid = f"sec{num}"
        label = sec['title']
        # Take first part before <br> if any
        label = re.sub(r'<br\s*/?>.*', '', label, flags=re.DOTALL)
        label = re.sub(r'<[^>]+>', '', label).strip()
        if len(label) > 10:
            label = label[:10]
        desc = sec['label']
        pos = positions[i % len(positions)]
        nodes.append(f"    {{id:'{sid}',label:'{label}',x:{pos[0]},y:{pos[1]},r:14,color:'#b8900a',cat:'section',url:'#{sid}',desc:'{desc}'}}")
        edges.append(f"    {{from:'self',to:'{sid}'}}")

    # Related moat page nodes (other 4 pages in the moat section)
    rel_positions = [(-170, -10, 12), (180, -5, 12), (-150, 110, 11), (170, 100, 11)]
    others = [(k, v) for k, v in MOAT_PAGES.items() if k != slug]
    for i, (rel_slug, rel_name) in enumerate(others[:4]):
        rid = f"rel{i}"
        rlabel = rel_name[:8]
        pos = rel_positions[i]
        href = f"../{rel_slug}/"
        nodes.append(f"    {{id:'{rid}',label:'{rlabel}',x:{pos[0]},y:{pos[1]},r:{pos[2]},color:'#2a3d6a',cat:'related',url:'{href}',desc:'{rel_name}'}}")
        edges.append(f"    {{from:'self',to:'{rid}'}}")

    # Extra cross edges between sections if >=2
    if len(sections) >= 2:
        edges.append("    {from:'sec01',to:'sec02'}")
    if len(sections) >= 4:
        edges.append("    {from:'sec03',to:'sec04'}")
...
(function(){{
  ...
  var catLabels={{self:'この記事',section:'セクション',related:'関連概念'}};
...
{edges_str}
  ...
  function draw(){{
    ...
    var legend=[{{l:'\\u3053\\u306e\\u8a18\\u4e8b',c:'#d4aa22'}},{{l:'\\u30bb\\u30af\\u30b7\\u30e7\\u30f3',c:'#b8900a'}},{{l:'\\u95a2\\u9023\\u6982\\u5ff5',c:'#2a3d6a'}}];
    var lx=8,ly=H-10;
...
def build_toc_html(sections):
    ...
    for i, sec in enumerate(sections):
        ...
        title = re.sub(r'<[^>]+>', ' ', sec['title']).strip()
        items.append(f'      <li><a href="#sec{num}"><span class="toc-num">{num}</span>{title}</a></li>')
    return "\n".join(items)
...
def process_page(filepath):
    ...
    if 'hero-3col' in html:
        print(f"  SKIP (already applied): {filepath}")
        return False
    ...
    if not hero['title']:
        print(f"  SKIP (no hero title): {filepath}")
        return False
    ...
    if not old_hero:
        print(f"  SKIP (hero pattern not matched): {filepath}")
        return False
  ...
  <div class="hero-3col">
    ...
    <div class="hero-col-side hero-col-left">
      <div class="graph-card">
        <div class="graph-card-label">KNOWLEDGE MAP</div>
...
<div class="nav-cockpit">
  <div class="cockpit-panel">
    ...
    <div class="cockpit-tree">
      <div class="ct-item ct-root"><span class="ct-icon">&#128218;</span><a href="../../">\u6295\u8cc7\u3068\u601d\u8003\u306e\u66f8\u658e</a></div>
      <div class="ct-children">
        <div class="ct-item ct-folder"><span class="ct-icon">&#128194;</span><a href="../">\u6982\u5ff5\u306e\u68da</a><span class="ct-count">5</span></div>
        <div class="ct-children">
...
{toc_html}
    ...
    footer_pattern = re.search(r'<footer class="footer">', html)
    if footer_pattern:
        html = html[:footer_pattern.start()] + MYNOTE_FAB_HTML + "\n" + html[footer_pattern.start():]

    # 7. Build and add graph JS + MyNote JS
...
"""
    ...
    if 'ai-assistant.js' not in html:
        scripts_block += '<script src="../../ai-assistant.js"></script>\n'
...
def main():
    ...
    for slug in sorted(MOAT_PAGES.keys()):
        filepath = os.path.join(MOAT_DIR, slug, 'index.html')
