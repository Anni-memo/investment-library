#!/usr/bin/env python3
"""
Researchページ「moat分析系5ページ」にもりもり版テンプレートの不足要素を追加するスクリプト
（cockpitは既存、hero-3col / graphCanvas / mynoteFab を追加する）

対象:
- research/buffett-shosha
- research/convenience-store-moat
- research/ohsho-moat
- research/sogo-shosha-capital
- research/tsmc-ecosystem-moat
...
TARGETS = [
    "buffett-shosha",
    "convenience-store-moat",
    "ohsho-moat",
    "sogo-shosha-capital",
    "tsmc-ecosystem-moat",
]

# スラッグ→短縮名（グラフ用・moat系記事）
SLUG_NAMES = {
    "buffett-shosha": "バフェット商社",
    "convenience-store-moat": "コンビニ",
    "ohsho-moat": "餃子の王将",
    "sogo-shosha-capital": "商社資本",
...
def extract_sections_from_toc(html):
    """既存の目次 (<ol class="toc-list">...) から抽出。
    以下の両パターンをサポート:
    - <li><a href="#X">title</a></li>
    - <li><span class="toc-num">01</span><a href="#X">title</a></li>
    """
    sections = []
    ...
    # li の中にある最初の <a href="#...">title</a> を拾う（間に span 等があってもOK）
    for li_m in re.finditer(r'<li[^>]*>(.*?)</li>', toc_body, re.DOTALL):
        inner = li_m.group(1)
...
def repair_previous_corruption(html):
    ...
    # 既に挿入された hero-3col ブロックを元の <div class="hero"><div class="hero-inner">…</div></div> に戻す
    hero3_re = re.compile(
        r'<!--[^>]*HERO \(3-column, Morimori\)[^>]*-->\s*<div class="hero">\s*<div class="hero-3col">.*?'
        r'<div class="hero-col-center">\s*(<div class="hero-inner">.*?</div>)\s*</div>.*?</div>\s*</div>',
        re.DOTALL,
    )
    html = hero3_re.sub(lambda m: f'<div class="hero">\n  {m.group(1)}\n</div>', html)
    return html


def patch_existing_qa_and_graph(html, slug, sections):
    """既に hero-3col 適用済みだが QA とグラフが空の場合、QA とグラフを差し替える。"""
    if not sections:
        return html, False
    changed = False

    # 1. Quick Actions の中身を差し替え
    qa_links = build_qa_links(sections)
    qa_re = re.compile(
        r'(<div class="quick-actions-v">)(.*?)(</div>)',
        re.DOTALL,
    )
    def qa_sub(m):
        return f"{m.group(1)}\n{qa_links}\n        {m.group(3)}"
    new_html, n = qa_re.subn(qa_sub, html, count=1)
    if n > 0:
        html = new_html
        changed = True

    # 2. Knowledge Graph の nodes/edges を差し替え
    # 既存のタイトルを再取得
    title_m = re.search(r'<h1 class="hero-title">(.*?)</h1>', html, re.DOTALL)
    title = title_m.group(1).strip() if title_m else ''
    graph_nodes, graph_edges = build_graph_nodes(title, sections, slug)
    new_graph_js = build_graph_js(graph_nodes, graph_edges)
    ...
    if new_core_m:
        new_html2, n2 = graph_block_re.subn(
            lambda _m: new_core_m.group(0), html, count=1
        )
        if n2 > 0:
            html = new_html2
            changed = True
...
def process_page(filepath):
    ...
    # 既に適用済み (hero-3col あり) の場合: QA とグラフを再生成するパッチモードで処理
    if 'hero-3col' in html:
        sections = extract_sections_from_toc(html)
        if not sections:
            print(f"  SKIP (already applied, still no TOC): {slug}")
            return False
        html, changed = patch_existing_qa_and_graph(html, slug, sections)
        if changed:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(html)
            print(f"  PATCHED: updated QA and graph with {len(sections)} sections")
            return True
        else:
            print(f"  SKIP (already applied, nothing to patch): {slug}")
            return False
